...
4. The agent spawner creates a run record for the ticket, then creates an Oz cloud agent run with the configured skill, model, and environment.
5. The run monitor polls running run records for completion, updates run-level state, recomputes ticket-level status, and transitions the Jira ticket.
...
| Agent Spawner | Constructs prompt, selects model/skill/environment, creates per-run `dispatch_runs` rows (`run_type` aware), then calls Oz SDK |
