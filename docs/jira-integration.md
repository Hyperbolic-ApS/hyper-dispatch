...
When GitHub sends a signed `pull_request` webhook with `action: "opened"` and `pull_request.draft: true`, HyperDispatch immediately attempts to mark that PR as ready-for-review (`draft: false`) so newly created worker PRs land in the open state.
