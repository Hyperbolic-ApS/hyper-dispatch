...
- `createRun(ticketKey, runType)` inserts a new run record for implementation spawn attempts (revision rows are now claimed via `claimRevisionSlot`; see below). New rows inherit the ticket's latest known PR metadata (`pr_url`, display state, conflict state, review/revision-running flags) so follow-up runs keep the PR linkage.
- `updateRunStatus` writes run-level fields and can target a specific run record id (`run_record_id`) so monitor/webhook updates only mutate the intended run row. If no target run row is found, it can create a fallback run from supplied fields (using the entry status when `status` is omitted) so metadata like `error`, `session_link`, and PR fields are not silently dropped. When PR fields are omitted, fallback inheritance reads from the ticket's latest run with known PR metadata.
  - With no explicit `run_record_id`, a missing target always falls back to inserting a new run row.
  - With an explicit `run_record_id`, fallback insertion only happens when the ticket already has run history (stale id against an existing ticket). If the ticket has no run history, `updateRunStatus` returns `null` and does not fabricate a run row.
...
- `claimRevisionSlot` atomically flips the entry to `running` **and inserts a `dispatch_runs` row (`run_type='revision'`, `status='running'`) in the same SQL statement** when no running run exists and the latest run is terminal, so concurrent revision triggers cannot both claim and there is no claim→create gap. The inserted revision row inherits the latest known PR metadata for the ticket, which keeps dashboard/API PR links stable while revision/review workflows run.
