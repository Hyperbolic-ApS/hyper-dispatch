  ...
  const rows = runs.map((run) => {
    ...
    const runHistoryRows = runHistory.length === 0
      ...
      : runHistory.map((historyRun) => {
          const openLink = historyRun.session_link
            ? `<a href="${historyRun.session_link}" target="_blank" class="run-history-open" aria-label="Open run ${historyRun.id} in new tab">↗</a>`
            : "";
          return `<li class="run-history-item">
            <button type="button" class="run-history-status-btn" data-run-history-pin data-ticket-key="${run.ticket_key}" aria-label="Pin run ${historyRun.id}">${statusBadge(historyRun.status)}</button>
    ...
    return `<tr>
      ...
      <td>
        <span class="agent-status-cell run-history-trigger" data-run-history-trigger>
          ...
          <span class="run-history-popover" role="dialog" aria-label="Run history for ${run.ticket_key}">
